...
const solution = () => {
  ...
  while (calculations.customersRemaining > 0) {
    ...
    if (calculations.prevDistance === 0) {
      ...
      let miles = getMiles(calculations.dosfec[closest].distance.value)
      routeObject.path = `${Number((miles).toFixed(1))} miles from Store to ${customers[closest]}.`
     ...
     */
      ...
      if (!skipForTest &&
        ...
        let last_customer = calculations.customersRemaining;

        const routeObject = {};
        /* Set the customer to the route */

        routeObject.customer = customers[last_customer];
        
        /* Get the charges for the customer using calculate charges method */
        routeObject.deliveryCharges = calculateCharges(calculations.dosfec[last_customer].distance.value);

        calculations.prevDistance = calculations.dosfec[last_customer].distance.value;

        let miles = getMiles(calculations.dosfec[last_customer].distance.value);

        routeObject.path = `${Number((miles).toFixed(2))} miles from ${customers[calculations.lastAllocated]} to ${customers[last_customer]}.`
        ...
        calculations.lastAllocated = last_customer++;
        ...
        let miles = getmiles(calculations.prevDistance);

        calculations.deliveryRoutes.push({ totalLength: `${(Number((miles).toFixed(1)))} miles from ${customers[calculations.lastAllocated]}`, route: calculations.currentRoute });
    ...
    if (calculations.customersRemaining < 1) {
      ...
      let miles = getMiles(calculations.prevDistance)
      calculations.deliveryRoutes.push({ totalLength: `${(Number((miles).toFixed(1)))} miles from Store.`, route: calculations.currentRoute });
      /* Set the current route be empty */
      calculations.currentRoute = [];
    }
  }
